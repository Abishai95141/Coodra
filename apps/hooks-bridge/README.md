...
Mirrors mcp-server's chain, sourced from `@contextos/shared/auth`:
...
```bash
pnpm --filter @contextos/hooks-bridge test:unit
...
- **Pre-tool latency is the single hardest budget** in this service. The policy module's cache + breaker (in `@contextos/policy`) is the load-bearing piece; do not bypass it.
