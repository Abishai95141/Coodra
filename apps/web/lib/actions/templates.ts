...
export async function installTemplateFromPathAction(formData: FormData): Promise<void> {
  ...
  redirect(
    `/projects/${encodeURIComponent(parsed.data.projectSlug)}/templates?installed=${encodeURIComponent(result.installed)}`,
  );
}

function redirectHref(projectSlug: string, errorCode: string, message: string): string {
  const search = new URLSearchParams();
  search.set('error', errorCode);
  search.set('errorMessage', message);
  return `/projects/${encodeURIComponent(projectSlug)}/templates?${search.toString()}`;
}
