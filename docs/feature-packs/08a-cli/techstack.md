...
| `zod` | `^4.3.6` (workspace pin) | Validating `.mcp.json`, `meta.json`, the `team join` token shape, env vars. Same pin as `@contextos/shared`. |
...
| `env-paths` | `^3.0.0` | Cross-platform path resolution per spec §11 Decision 2 (XDG on Linux when `$XDG_CONFIG_HOME` is set, `$HOME/.contextos/` default elsewhere). Configured with `{ suffix: '' }` to drop the default `-nodejs` suffix so the path is exactly `~/.contextos/` (or `$XDG_CONFIG_HOME/contextos/` when XDG is set). Does NOT resolve `%APPDATA%\contextos\` on Windows by default, which matches Decision 2's "$HOME default elsewhere including Windows" line. **Considered alternative: hand-rolled `path.join(homedir(), '.contextos')`** — would work for non-XDG platforms but loses the XDG-on-Linux check. Locked decision: ship `env-paths`. |
| `@contextos/shared` | workspace | Logger, errors, env validation. The CLI MUST use the same pino logger and the same error hierarchy as every other package. |
| `@contextos/db` | workspace | The `doctor` check that probes `~/.contextos/data.db` reuses the existing SQLite open path so behavior matches what the MCP server expects. |
...
| OS | Strategy | Why |
|---|---|---|
| macOS | `launchd` via `launchctl` (writes a `~/Library/LaunchAgents/com.contextos.<unit>.plist`) | macOS native, runs on user login, restart-on-crash supported, no root needed for user agents. |
| Linux | `systemd --user` (writes `~/.config/systemd/user/contextos-<unit>.service`) | Native, no root needed, `Restart=on-failure` baked in, journal logs. Falls back to `fallback.ts` on Linux distros where user systemd isn't enabled (rare on modern desktop distros, common on minimal containers). |
...
| All | `fallback.ts` (detached child + PID file under `~/.contextos/pids/`) | Universal fallback when the platform's native manager is unreachable. |
...
|---|---|
| `npm` (scope: `@contextos`, package: `cli`) | Package builds and runs locally. **Not published in 08a.** Publish-flag-day is a separate ops task tracked in `pending-user-actions.md`. |
...
- `dotenv` — the CLI does not read `.env` files. It reads `process.env` directly and `~/.contextos/config.json` for team-mode secrets (when team mode opens). Project-level `.env` is the application's concern, not the installer's.
...
- **`npx @contextos/cli init` resolves the bin via `process.execPath` for the `.mcp.json` `command` field.** This means the `.mcp.json` written by an `npx` invocation embeds a path under the npx cache, which can be garbage-collected by npm. The `init` command MUST detect this case and warn YELLOW in `doctor`'s "MCP config validity" check, recommending a global install (`npm i -g @contextos/cli`) for stability. (Tracked as a follow-up: rewrite the `.mcp.json` `command` to `npx @contextos/cli mcp-stdio` so the path is resolved at IDE-startup time, not at `init` time. Out of 08a scope.)
