...
5. `contextos doctor` runs ≥10 health checks (Node version, pnpm presence if needed, port availability, `~/.contextos/data.db` writability, `.mcp.json` validity, IDE detection, daemon-manager reachability, etc.) and prints a numbered report with **green/yellow/red** status per check + a one-line remediation when red.
...
| `contextos team login <token> [--server <url>]` | **Stub in 08a** per §11 Decision 1 — full flag set + help text + exit-code 2 message ("team mode not yet generally available"). Naming locked to `team login` (matches M02 §8.6 carryover; "login" reads more naturally than "join" in the auth context). Body — invite-token exchange + `~/.contextos/config.json` write (mode `0600`) per §19 — lands when team mode is reachable end-to-end. | 0 (when implemented) / 2 (08a stub) |
| `contextos team logout` | **Stub in 08a** per §11 Decision 1 — same surface, same exit-2 stub. Body (rotate local secret + clear `~/.contextos/config.json`) lands alongside `team login`. | 0 (when implemented) / 2 (08a stub) |
| `contextos --version` | Prints the npm version. | 0 |
| `contextos --help` / `contextos <cmd> --help` | Help text. | 0 |
...
|---|---|---|
| `~/.contextos/data.db` | Local SQLite primary store | empty file with all migrations applied + `__global__` sentinel project seeded (F7) |
| `~/.contextos/config.json` | Team-mode `LOCAL_HOOK_SECRET` (mode `0600` per §19) | **absent after `init`** — only `team login` writes this file |
| `<repo>/.env` (or merge) | Solo-mode env vars | `CONTEXTOS_MODE=solo`, `CLERK_SECRET_KEY=sk_test_replace_me` (sentinel from §19), `CLERK_PUBLISHABLE_KEY=pk_test_replace_me`, `LOCAL_HOOK_SECRET=<random hex generated at init time, solo-only>`, `MCP_SERVER_PORT=3100`, `HOOKS_BRIDGE_PORT=3101`. **Not written:** `ANTHROPIC_API_KEY`, `GOOGLE_API_KEY`, `OPENAI_API_KEY`, `GITHUB_APP_*`, `ATLASSIAN_*`, `SUPABASE_*`, `UPSTASH_*` — all are user-action items per §2.2 and the existing `pending-user-actions.md` entries. |
| `<repo>/.contextos.json` | Bridge's `projectSlugResolver` config | `{ "projectSlug": "<derived or --project-slug>" }` |
| `<repo>/.mcp.json` | Claude Code / IDE MCP server registration | merges a `contextos` entry into `mcpServers`; existing servers preserved; conflicting `contextos` entry triggers idempotent merge per §11 Decision 3 (existing entry inspected and left alone if already correct, drift triggers merge, `--force` overrides to baseline) |

The `LOCAL_HOOK_SECRET` written by `init` to a project-local `.env` is **solo-mode-only** and per-project. Team mode reuses the user-level `~/.contextos/config.json` value across projects. Both are random hex generated by `crypto.randomBytes(32).toString('hex')`; neither is ever a hardcoded sentinel (per §2.4 — secrets the agent generates locally for its own dev bypass are fine; secrets that authenticate against a real service are not).

## 4.5 `contextos doctor` — what each check surfaces
...
| 8 | Bridge `pre_tool_use_decision` log lines from the last 24h include `runId` field (F15 spot-check via `~/.contextos/logs/`) | yellow | F15 closure live; reads the bridge's pino-rotated logs. |
...
| 12 | Project registered for `cwd` — `.contextos.json` parses, slug resolves to a `projects` row | yellow | Pre-condition for governance — without this, the bridge falls to `__global__` (still audited per F7 but project-specific rules don't fire). |
...
| 14 | `.mcp.json` parses as JSON if present + the `contextos` entry's `command` path resolves on disk | yellow | Catches the `npx`-cache-GC footgun named in `techstack.md` Gotchas. |
...
| 17 | Port 3100 (MCP) availability when nothing is registered as `start`ed | yellow | Catches a stray non-ContextOS process binding the port. |
...
| 20 | `LOCAL_HOOK_SECRET` set (env or `~/.contextos/config.json`) AND length ≥ 32 hex chars | yellow | Bridge auth chain pre-condition. The CLI does NOT print the secret — only its length and source. |
...
## 4.6 `contextos status` — unified state in one screen
...
Run `contextos doctor` for the full diagnostic.
```
...
✓ Registered launchd entry: com.contextos.local (auto-starts at login)

ContextOS is ready.
  → Open Claude Code → run /mcp to verify.
  → Run `contextos doctor` if anything misbehaves.
  → First Context Pack saves automatically at session end.
```

A user who has never read the ContextOS docs can complete this. That is the bar.
...
`system-architecture.md §13` already describes the shape of `contextos start` for solo mode. Module 08a is the implementation of that section. Where §13 says "Process management: PIDs written to `~/.contextos/pids`", Module 08a expands that into the platform-native daemon-manager integration (launchd / systemd / Task Scheduler) and amends §13 in the same commit per amendment B.

`system-architecture.md §19` says `LOCAL_HOOK_SECRET` lives in `~/.contextos/config.json`. Module 08a's `team join` command is the writer of that file (in a future slice when team mode reaches GA); the CLI's read-path for 08a remains env-var-only because the OAuth round-trip is not yet wired. The `pending-user-actions.md` entry "LOCAL_HOOK_SECRET config-file reads" updates to point at this module.

## 8. What "done" hands off to Modules 04 and 07

- A working `contextos init` that downstream module specs can refer to as "the install path" without qualification.
- A working `contextos start` / `stop` lifecycle that the VS Code Extension (Module 07) shells out to for service control.
- A `contextos doctor` checklist that the Web App (Module 04) can render in its admin panel via JSON output.
...
### Decision 1 — `contextos team login` ships as a stub in 08a
...
- **What this constrains:** S8 ships the stub; the secret-write path is explicitly NOT in 08a's surface. `pending-user-actions.md`'s "LOCAL_HOOK_SECRET config-file reads via a future contextos team login CLI" entry updates to "command surface lives in 08a; OAuth round-trip lands when team mode opens."

### Decision 2 — XDG on Linux when set, `$HOME/.contextos/` default elsewhere

- **Question (was):** Where does `~/.contextos/` resolve cross-platform?
- **Decision:** Honor `$XDG_CONFIG_HOME` on Linux when set, default to `$HOME/.contextos/` everywhere otherwise (including Windows). Resolution lives in a single path helper using `env-paths` (configured `{ suffix: '' }` to drop the default `-nodejs` suffix).
- **Why this answer:** Linux XDG-respect costs ~5 lines and matches the platform's documented convention; Linux desktop environments increasingly enforce XDG. Windows `%APPDATA%` integration is genuinely non-trivial (paths cross drive letters, forward-slash support varies in npm-shipped Node tools) — defer to a future slice if a Windows user complains. The architecture spec's `~/.contextos/` mentions stay correct because they specify the default; the XDG override is a feature, not a contradiction. `system-architecture.md §1` gets a same-commit clarification at S5.
- **What this constrains:** S5 reads the path through `env-paths`, never hardcodes `path.join(os.homedir(), '.contextos')`. S3 doctor uses the same helper so the check that probes `~/.contextos/data.db` finds the file at the right place on Linux+XDG.

### Decision 3 — Idempotent merge by default; `--force` is the destructive override

- **Question (was):** What's the `init` re-run behavior?
- **Decision:** Re-run inspects each file (`.mcp.json`, `.contextos.json`, `<repo>/.env`, `docs/feature-packs/<slug>/`), leaves it alone if already correct, merges only if drift detected, never destroys user edits. `--force` overrides to baseline (destructive — writes the baseline, loses user edits). Re-run with no flags is always safe.
- **Why this answer:** Matches the M02 verification F13 closure mindset (`save_context_pack` defaults to `~/.contextos/packs/` so re-runs are non-destructive by default). Re-running `init` after a pack update, a Module-04-led reinstall, or a CI provisioning step must be safe. Option (b) prompt-and-skip-per-file would re-introduce the `prompts` dep that techstack.md declines — and friendly prompts don't compose with `--json` / non-TTY use. Option (c) hard-refuse-without-`--force` is harsh on first-time re-runners and makes scripted setups fragile.
...
- **What this constrains:** S1 `package.json` includes `bin` + `files` + `engines` + `repository` + `publishConfig` from day one. S9 locks the file list. The contributor dev-loop (DEVELOPMENT.md §"Iterating on the CLI") describes how to invoke the CLI without `npm i -g`: `pnpm --filter @contextos/cli build && pnpm --filter @contextos/cli exec contextos init`. No new build tool — same TS pipeline as every other workspace package.
